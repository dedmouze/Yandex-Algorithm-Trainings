...
int find(memory_struct& memory, int root, int x)
{
	...
	else if (x < key)
	{
		int left = memory.nodes[root].left;
		if (left == -1) return -1;
		else find(memory, left, x);
	}
	else if (x > key)
	{
		int right = memory.nodes[root].right;
		if (right == -1) return -1;
		else find(memory, right, x);
	}
}

int get_height(memory_struct& tree, int root, int height = 0)
{
	if (root == -1) return height;

	return std::max(get_height(tree, tree.nodes[root].left, height + 1), 
					get_height(tree, tree.nodes[root].right, height + 1));
}
